...
function get_ai_output(button) {
    ...
    if (prompt) {
        ...
        fetch('http://localhost:1234/api/v0/chat/completions', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system", 
                        content: `Você é um assistente de escrita para uma Wiki. 
        ...
        .then(data => {
            const ai_output = data.choices[0].message.content;
            console.log(data.choices.json())
            const ai_content_block = document.getElementsByClassName("ai-content-block")[0]
            ai_content_block.firstElementChild.innerHTML = ai_output
